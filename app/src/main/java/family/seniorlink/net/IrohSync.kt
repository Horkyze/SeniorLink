                    ...
                    jobs[peer] = launch {
                        ...
                        while (isActive && store.approved(peer)) {
                            try {
                                ...
                                val more = EndpointId.fromString(peer).use { id ->
                                    EndpointAddr(id, null, emptyList()).use { addr ->
                                        val connection = withTimeout(35_000) { endpoint.connect(addr, Wire.ALPN) }
                                        try {
                                            check(connection.remoteId().use { it.toString() } == peer)
                                            catchUpConnection(peer, store, Session(connection)) {
                                                status(peer, "Fetching history…")
                                            }
                                        } finally {
