...
- Install 0.1.12 on all paired phones. Verify in-place updates preserve pairing,
...
- In 0.1.12, leave Updates open as a new pulse arrives. Its graph should include
  the received reading immediately, including on the sharing phone before the
  two-minute summary is saved. Readings older than one hour stay outside that view.
- Tap Phone battery for the recorded day's chart. Inspect several points and
  compare percentage, timestamp and charging status with saved records. Check
  the 0–100% scale, missing-status label, gaps longer than ten minutes, and 1/24-hour
  views on the Wearable screen. Charging status describes the recorded time;
  plugging in does not imply an immediate remote update.
